        ...
        $(".operateChange").on('click',function(){//如果点击修改则跳转到个人页面
          ...
          $.ajax({//发送请求获取个人信息
            ...
            success:function(data){
              ...
              window.setTimeout(function(){$("#alert").remove();},2000);
              $(".fromPart input").eq(0).attr({ value: data[0].name });
              $(".fromPart input").eq(1).attr({ value: data[0].department });
              $(".fromPart input").eq(2).attr({ value: data[0].position });
              $(".fromPart input").eq(3).attr({ value: data[0].tel });
              $(".fromPart input").eq(4).attr({ value: data[0].QQ });
              $(".fromPart input").eq(5).attr({ value: data[0].email });
              $(".fromPart input").eq(6).attr({ value: data[0].school });
              $(".fromPart input").eq(7).attr({ value: data[0].number });
              $(".fromPart input").eq(8).attr({ value: data[0].birthday });
            },
            error:function(data){

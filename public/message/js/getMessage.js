	...
	if(getUrlParam("way")=="addressBook"){//当从通讯录进入个人信息时的情况
		...
		$(function(){/*载入时获得人员的数据*/
          ...
          queryNumber:getUrlParam("queryNumber")
				...
				success:function(data){
					$("#callback").hide();
					$('input').eq(0).attr({value: data[0].name});
					$('input').eq(1).attr({value: data[0].school});
					$('input').eq(2).attr({value: data[0].department});
					$('input').eq(3).attr({value: data[0].position});
					$('input').eq(4).attr({value: data[0].birthday});
					$('input').eq(5).attr({value: data[0].tel});
					$('input').eq(6).attr({value: data[0].QQ});
					$('input').eq(7).attr({value: data[0].email});
					$('input').eq(8).attr({value: data[0].number});
					$('userTextarea').attr({value: data[0].message});
			  },
			  error:function(data){
